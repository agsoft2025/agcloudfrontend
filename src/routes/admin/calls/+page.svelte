...
<script lang="ts">
  ...
  $: filteredCalls = applyFilters(allCalls, filterFrom, filterTo, filterType, filterMode);

  // Reset to page 1 whenever the filtered set changes
  $: filteredCalls, (currentPage = 1);

  $: totalPages  = Math.max(1, Math.ceil(filteredCalls.length / PAGE_SIZE));
  $: pagedCalls  = filteredCalls.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);

  $: rangeStart  = filteredCalls.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1;
  ...
  $: hasFilter = filterFrom !== '' || filterTo !== '' || filterType !== '' || filterMode !== '';

  // ── Helpers ───────────────────────────────────────────────────

  function resolveDisplayName(userId?: string, fallbackName?: string | null): string {
  ...
  function resolveAvatar(userId?: string, fallbackAvatar?: string | null): string | undefined {
  ...
  function statusBadgeClass(entry: CallHistoryEntry): string {
    ...
    if (s === 'missed' || entry.participantStatus === 'missed') return 'badge--missed';
    if (s === 'rejected' || entry.participantStatus === 'rejected') return 'badge--rejected';
  ...
  function statusLabel(entry: CallHistoryEntry): string {
    ...
    if (s === 'rejected' || entry.participantStatus === 'rejected') return 'Rejected';
  ...
  function hydratePeerProfiles(entries: CallHistoryEntry[]) {
    ...
    for (const e of entries) {
      ...
      if (e.receiverIds) e.receiverIds.forEach((id) => ids.add(id));
    }
    for (const id of ids) {
      if (!userStore.getProfile(id) && !userStore.isLoading(id)) {
        void userStore.hydrateProfile(id).catch(() => {});
      }
    }
  }
  ...
  async function load() {
    ...
    error = null;
    try {
      allCalls = await getCallHistory(500);
      hydratePeerProfiles(allCalls);
    } catch (err) {
...
<div class="admin-page">
  ...
  <div class="filter-bar" role="search" aria-label="Filter calls">
    <div class="filter-fields">
      ...
      <label class="filter-label">
        ...
        <select class="filter-select" bind:value={filterType} aria-label="Call type">
      ...
      <label class="filter-label">
        ...
        <select class="filter-select" bind:value={filterMode} aria-label="Call mode">
          <option value="">All</option>
    ...
    {#if !isLoading && hasFilter}
      <span class="filter-result-count" aria-live="polite">
        {filteredCalls.length} of {allCalls.length} call{allCalls.length === 1 ? '' : 's'}
      </span>
    {/if}
  </div>
  ...
  {:else if error}
    <div class="state-box state-box--error" role="alert">
      ...
      <p>{error}</p>
      <button class="retry-btn" type="button" on:click={() => void load()}>Retry</button>
    </div>
  ...
  {:else if allCalls.length === 0}
    <div class="state-box" role="status">
      <svg width="32" height="32" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.72 12 19.79 19.79 0 0 1 1.65 3.32 2 2 0 0 1 3.63 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6.35 6.35l.98-.97a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7a2 2 0 0 1 1.72 2.03z"
  ...
  {:else if filteredCalls.length === 0}
    <div class="state-box" role="status">
      ...
      <button class="retry-btn" type="button" on:click={clearFilter}>Clear filter</button>
    </div>
  ...
  {:else}
      ...
      <table class="call-table" aria-label="Call list">
        ...
        <tbody>
          {#each pagedCalls as entry (entry.id)}
            {@const callerName   = resolveDisplayName(entry.callerId, entry.callerName)}
            {@const calleeName   = resolveDisplayName(entry.calleeId ?? entry.receiverIds?.[0], entry.calleeName)}
            {@const callerAvatar = resolveAvatar(entry.callerId, entry.callerAvatar)}
            {@const calleeAvatar = resolveAvatar(entry.calleeId ?? entry.receiverIds?.[0], entry.calleeAvatar)}
            <tr class:row--active={entry.isActive}>
              ...
              <td>
                <span class="type-chip type-chip--{entry.callType ?? 'unknown'}">
              ...
              <td>
                <span class="badge {statusBadgeClass(entry)}">
                  {#if entry.isActive}<span class="live-dot" aria-hidden="true"></span>{/if}
                  {statusLabel(entry)}
                </span>
              </td>
    ...
    <div class="pagination" aria-label="Pagination">
      <span class="pagination-info">
        {rangeStart}–{rangeEnd} of {filteredCalls.length} call{filteredCalls.length === 1 ? '' : 's'}
      </span>
...
<style lang="postcss">
  ...
  .filter-input:hover  { border-color: var(--color-border-strong); }
  .filter-input:focus  { outline: 2px solid var(--color-secondary); outline-offset: 1px; border-color: transparent; }
  ...
  .call-table tbody tr:last-child { border-block-end: none; }
  .call-table tbody tr:hover { background: color-mix(in srgb, var(--color-primary) 4%, transparent); }
  .call-table tbody tr.row--active { background: color-mix(in srgb, #38a169 6%, transparent); }
  ...
  .cell-date { white-space: nowrap; opacity: 0.75; font-variant-numeric: tabular-nums; }
  .cell-dur  { white-space: nowrap; font-variant-numeric: tabular-nums; opacity: 0.85; }
  .cell-mode { opacity: 0.75; text-transform: capitalize; }
  ...
  .badge--active   { background: color-mix(in srgb, #38a169 15%, transparent); color: #276749; }
  .badge--ended    { background: color-mix(in srgb, var(--color-text) 8%, transparent); color: var(--color-text); opacity: 0.7; }
  .badge--missed   { background: color-mix(in srgb, #e53e3e 12%, transparent); color: #9b2c2c; }
  .badge--rejected { background: color-mix(in srgb, #dd6b20 12%, transparent); color: #7b341e; }
  .badge--ringing  { background: color-mix(in srgb, #3182ce 12%, transparent); color: #2c5282; }
  .badge--default  { background: color-mix(in srgb, var(--color-text) 8%, transparent); color: var(--color-text); }
  ...
  .page-btn:focus-visible { outline: 2px solid var(--color-secondary); outline-offset: 1px; }
